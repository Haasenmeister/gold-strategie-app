...
with col_main:
    if st.button("🔍 ALLE ROHSTOFFE ANALYSIEREN", use_container_width=True):
        ...
        for name, ticker in engine.assets.items():
            ...
            sig = engine.get_signal(name, gmsi.iloc[-1], gmsi_sma, ratio, ratio_sma, vix_val, rsi)
        ...
        if num_active > 0:
            ...
            st.warning("Keine klaren Signale nach RSI-Schutzprüfung.")
        ...
        for res in results:
            ...
            with st.expander(f"{'✅ ' if is_ready else '📊 '}{name} ANALYSE (RSI: {rsi:.1f})", expanded=is_ready):
                if is_ready:
        ...
        st.divider()
        st.subheader("🔗 Korrelations-Check (60 Tage)")
