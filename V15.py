...
st.set_page_config(page_title="Multi-Commodity Ultra Terminal Lvl 4.3", layout="wide")
...
st.markdown("""
    ...
    .t212-highlight { color: #3b82f6; font-size: 1.5rem; font-weight: bold; }
...
class TradingEngine:
    ...
    def fetch_data(self):
        tickers = list(self.weights_std.keys()) + list(self.assets.values()) + ['^VIX', '^TNX', 'CHFUSD=X']
    ...
    def check_smart_entry(self, ticker, direction):
        ...
        last = float(h1_data.iloc[-1].iloc[0]) if isinstance(h1_data.iloc[-1], pd.Series) else float(h1_data.iloc[-1])
        prev = float(h1_data.iloc[-2].iloc[0]) if isinstance(h1_data.iloc[-2], pd.Series) else float(h1_data.iloc[-2])
        return last <= prev if "LONG" in direction else last >= prev

    def get_signal(self, name, current_gmsi, gmsi_sma, ratio, ratio_sma, vix, rsi, yields):
        yields_rising = yields.diff().iloc[-1] > 0.02
        pattern = "Neutral"
        if current_gmsi < gmsi_sma and ratio > ratio_sma: pattern = "KRISEN-MUSTER"
        elif current_gmsi > gmsi_sma and ratio > ratio_sma and rsi > 50: pattern = "INFLATIONS-MUSTER"
        
        if name in ['GOLD', 'SILBER', 'PLATIN']:
            if (current_gmsi < gmsi_sma or vix > 25) and ratio > ratio_sma and rsi < 70:
                return ('LONG (Zins-Warnung)' if yields_rising else 'LONG'), pattern
            if current_gmsi > gmsi_sma and ratio < ratio_sma and rsi > 30: return 'SHORT', pattern
        elif name in ['WTI_ÖL', 'KUPFER']:
            if current_gmsi > gmsi_sma and ratio < ratio_sma and rsi < 70: return 'LONG', pattern
            if current_gmsi < gmsi_sma and ratio > ratio_sma and rsi > 30: return 'SHORT', pattern
        return 'HOLD', pattern
...
st.title("💰 Multi-Commodity Ultra Terminal Lvl 4.3")
...
with col_main:
    if st.button("🚀 LEVEL 4 VOLL-ANALYSE STARTEN"):
        ...
        for name, ticker in engine.assets.items():
            w = engine.weights_crisis if vix_val > 25 else (engine.weights_eco if name in ['KUPFER', 'WTI_ÖL'] else engine.weights_std)
            ...
            sig, pat = engine.get_signal(name, gmsi_series.iloc[-1], gmsi_series.rolling(20).mean().iloc[-1], price/gmsi_series.iloc[-1], (price_s/gmsi_series).rolling(20).mean().iloc[-1], vix_val, rsi, yields_val)
            smart = engine.check_smart_entry(ticker, sig)
            results.append({'name': name, 'price': price, 'atr': atr, 'sig': sig, 'rsi': rsi, 'ratio': price/gmsi_series.iloc[-1], 'ratio_sma': (price_s/gmsi_series).rolling(20).mean().iloc[-1], 'pattern': pat, 'smart': smart})
    ...
    if st.session_state['analysis_results']:
        ...
        for res in res_list:
            ...
            clean_dir = "LONG" if "LONG" in sig or (sig == "HOLD" and res['ratio'] > res['ratio_sma']) else "SHORT"
            ...
            with st.expander(f"{'🔵 ' if is_active else '✅ ' if 'HOLD' not in sig else '📊 '}{name} | {pat}", expanded=(sig != 'HOLD' or is_active)):
                ...
                decision = f"GLASKLARE ANTWORT: {decision_sig}" if (decision_sig != "ABWARTEN" and smart) else f"GLASKLARE ANTWORT: {decision_sig} (SMART ENTRY WARTEN)" if decision_sig != "ABWARTEN" else "GLASKLARE ANTWORT: ABWARTEN"
                ...
                hebel = 7 if vola_p < 0.8 else 5 if vola_p < 1.5 else 2 if vola_p < 2.5 else 1
                m4.metric("Hebel", f"{hebel}x")

                st.markdown("---")
                einsatz_chf = start_kap / len(active_sigs) if active_sigs else start_kap
                if name in {'GOLD', 'SILBER', 'PLATIN'} and len(metal_sigs) > 1: einsatz_chf *= 0.7
                
                t212_wert = einsatz_chf * hebel
                sl, tp, tsl = (price - atr*2, price + atr*4, price - atr*1.5) if clean_dir == "LONG" else (price + atr*2, price - atr*4, price + atr*1.5)

                t1, t2, t3 = st.columns(3)
                t1.markdown(f'<p class="order-label">T212 POSITION-WERT (CHF)</p><p class="t212-highlight">{t212_wert:.0f}</p>', unsafe_allow_html=True)
                t2.markdown(f'<p class="order-label">Einsatz ohne Hebel</p><p class="order-value">{einsatz_chf:.2f} CHF</p>', unsafe_allow_html=True)
                t3.markdown(f'<p class="order-label">Richtung</p><p class="order-value">{clean_dir}</p>', unsafe_allow_html=True)

                st.info(f"SL: ${sl:.2f} | TP: ${tp:.2f} | Trailing Stop: ${tsl:.2f}")
                ...
                if not is_active:
                    ...
                    if st.button(f"CLOSE {name}", key=f"s_{name}"):
