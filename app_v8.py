...
with col_main:
    if st.button("🔍 ALLE ROHSTOFFE ANALYSIEREN", use_container_width=True):
        ...
        for name, ticker in engine.assets.items():
            ...
            results.append({'name': name, 'price': price, 'ratio': ratio, 'ratio_sma': ratio_sma, 'atr': atr, 'sig': sig, 'rsi': rsi, 'mode': mode})
        ...
        if vix_val > 25:
            st.error(f"⚠️ **MARKT-PANIK ERKANNT (VIX: {vix_val:.2f})** - System schaltet auf US-Krisen-Gewichtung um.")
        if num_active > 0:
            st.success(f"🚀 **READY TO INVEST:** {num_active} Signal(e) gefunden. Kapital pro Trade: **{capital_per_trade:.2f} CHF.**")
        for res in results:
            name, price, atr, sig, rsi, mode = res['name'], res['price'], res['atr'], res['sig'], res['rsi'], res['mode']
            vola = (atr / price) * 100
            ...
            is_ready = sig != 'HOLD'
            with st.expander(f"{'✅ ' if is_ready else '📊 '}{name} ANALYSE (Modus: {mode})", expanded=is_ready):
                ...
                r1_3.markdown(f'<p class="order-label">Typ</p><p class="order-value">Markt</p>', unsafe_allow_html=True)
                sl_v = price - (atr * 2) if final_dir == 'LONG' else price + (atr * 2)
                tp_v = price + (atr * 4) if final_dir == 'LONG' else price - (atr * 4)
                st.warning(f"Stop-Loss: **${sl_v:.2f}** | Take-Profit: **${tp_v:.2f}** | Exit: **{ctx['exit']} Uhr**")
        st.divider()
