...
class TradingEngine:
    def __init__(self):
        ...
        self.assets = {'GOLD': 'GC=F', 'SILBER': 'SI=F', 'WTI_ÖL': 'CL=F'}
    ...
    def get_pattern_analysis(self, asset_change, gmsi_change, atr, price):
        vola = (atr / price) * 100
        is_ultra_crisis = gmsi_change < -0.02 and asset_change > 0.01
        is_crisis = gmsi_change < -0.01 and asset_change >= 0
        is_inflation = gmsi_change > 0.01 and asset_change > 0.01
        
        if is_ultra_crisis and vola < 1.0:
            return 8, "ULTRA-KRISEN-MUSTER (8x)"
        elif is_crisis or is_inflation:
            return 5, "STARKES MUSTER (5x)"
        else:
            return 1, "KEIN MUSTER (1x)"
...
with col_depot:
    ...
    st.divider()
    with st.form("logger"):
        log_asset = st.selectbox("Rohstoff", ["GOLD", "SILBER", "WTI_ÖL"])
...
with col_main:
    if st.button("🔍 ANALYSE STARTEN", use_container_width=True):
        ...
        gmsi_now = gmsi.iloc[-1]
        gmsi_prev = gmsi.iloc[-2]
        gmsi_change = (gmsi_now - gmsi_prev) / gmsi_prev
        ...
        for name, ticker in engine.assets.items():
            ...
            price_prev = price_s.iloc[-2]
            asset_change = (price - price_prev) / price_prev
            
            ratio = price / gmsi_now
            ratio_sma = (price_s / gmsi).rolling(20).mean().iloc[-1]
            atr = (price_s.rolling(2).max() - price_s.rolling(2).min()).rolling(14).mean().iloc[-1]
            
            sig = engine.get_signal(name, gmsi_now, gmsi_sma, ratio, ratio_sma)
            lev, pattern_desc = engine.get_pattern_analysis(asset_change, gmsi_change, atr, price)
            
            results.append({
                'name': name, 'price': price, 'ratio': ratio, 
                'ratio_sma': ratio_sma, 'atr': atr, 'sig': sig, 
                'lev': lev, 'pattern': pattern_desc
            })
        ...
        for res in results:
            name, price, atr, sig, lev, pattern = res['name'], res['price'], res['atr'], res['sig'], res['lev'], res['pattern']
            is_ready = sig != 'HOLD'
            
            with st.expander(f"{'✅ ' if is_ready else '📊 '}{name} - {pattern}", expanded=is_ready):
                m1, m2, m3, m4 = st.columns(4)
                m1.metric("Preis", f"${price:.2f}")
                m2.metric("Hebel", f"{lev}x")
                m3.metric("Signal", sig)
                m4.metric("Kapital", f"{capital_per_trade:.2f} CHF")

                if is_ready:
                    final_dir = sig
                    sl_v = price - (atr * 1.5) if final_dir == 'LONG' else price + (atr * 1.5)
                    tp_v = price + (atr * 3.0) if final_dir == 'LONG' else price - (atr * 3.0)
                    ...
                    r1_2.markdown(f'<p class="order-label">Order (inkl. Hebel)</p><p class="order-value">{(capital_per_trade * lev):.2f} CHF</p>', unsafe_allow_html=True)
                    r1_3.markdown(f'<p class="order-label">Hebel-Typ</p><p class="order-value">{lev}x</p>', unsafe_allow_html=True)
                    ...
                    r2_3.markdown(f'<p class="order-label">Exit-Zeit</p><p class="order-value">{ctx["exit"]} Uhr</p>', unsafe_allow_html=True)
