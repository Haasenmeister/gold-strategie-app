...
st.set_page_config(page_title="Multi-Commodity Ultra Terminal Lvl 4.1", layout="wide")
...
class TradingEngine:
    ...
    def check_smart_entry(self, ticker, direction):
        ...
        if len(h1_data) < 2: return True
        if direction == "LONG":
            return h1_data.iloc[-1] <= h1_data.iloc[-2]
...
st.title("💰 Multi-Commodity Ultra Terminal Lvl 4.1")
...
with col_depot:
    ...
    start_kap = st.number_input("Kapital (CHF)", value=5000.0)
    if st.session_state['active_trades']:
        for t_name, t_info in st.session_state['active_trades'].items():
...
with col_main:
    if st.button("🚀 LEVEL 4 VOLL-ANALYSE STARTEN"):
        ...
        for name, ticker in engine.assets.items():
            ...
            sig, pat = engine.get_signal(name, gmsi_val, gmsi_sma, ratio, ratio_sma, vix_val, rsi, yields_val)
            smart = engine.check_smart_entry(ticker, sig if "LONG" in sig else "SHORT" if "SHORT" in sig else "LONG")
            results.append({'name': name, 'price': price, 'atr': atr, 'sig': sig, 'rsi': rsi, 'ratio': ratio, 'ratio_sma': ratio_sma, 'pattern': pat, 'smart': smart})
    ...
    if st.session_state['analysis_results']:
        ...
        for res in res_list:
            name, price, sig, rsi, atr, pat, smart = res['name'], res['price'], res['sig'], res['rsi'], res['atr'], res['pattern'], res['smart']
            is_active = name in st.session_state['active_trades']
            
            with st.expander(f"{'🔵 ' if is_active else '✅ ' if sig != 'HOLD' else '📊 '}{name} | {pat}", expanded=(sig != 'HOLD' or is_active)):
                if is_active:
                ...
                clean_sig = "LONG" if "LONG" in sig else "SHORT" if "SHORT" in sig else "HOLD"
                
                decision = f"GLASKLARE ANTWORT: {clean_sig}" if (clean_sig != "HOLD" and smart) else f"GLASKLARE ANTWORT: {clean_sig} (WARTE AUF SMART ENTRY)" if clean_sig != "HOLD" else "GLASKLARE ANTWORT: ABWARTEN"
                ...
                sl, tp, tsl = (price - atr*2, price + atr*4, price - atr*1.5) if "LONG" in final_dir else (price + atr*2, price - atr*4, price + atr*1.5)

                t1, t2, t3 = st.columns(3)
                t1.markdown(f'<p class="order-label">Richtung</p><p class="order-value">{final_dir}</p>', unsafe_allow_html=True)
                ...
                st.warning(f"T212 Trailing Stop Wert: ${tsl:.2f}")

                if not is_active:
                    if st.button(f"BESTÄTIGE {final_dir}: {name}", key=f"b_{name}"):
                        st.session_state['active_trades'][name] = {'entry': price, 'dir': "LONG" if "LONG" in final_dir else "SHORT"}
                        st.rerun()
                else:
                    if st.button(f"VERKAUF/CLOSE {name}", key=f"s_{name}"):
