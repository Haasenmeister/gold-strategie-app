...
st.set_page_config(page_title="Multi-Commodity Ultra Terminal Lvl 4.2", layout="wide")
...
class TradingEngine:
    ...
    def check_smart_entry(self, ticker, direction):
        ...
        if "LONG" in direction: return last_price <= prev_price
        if "SHORT" in direction: return last_price >= prev_price
        return True
    ...
    def get_signal(self, name, current_gmsi, gmsi_sma, current_ratio, ratio_sma, vix, rsi, yields):
        ...
        if name in ['GOLD', 'SILBER', 'PLATIN']:
            ...
            if current_gmsi > gmsi_sma and current_ratio < ratio_sma and rsi > 30: return 'SHORT', pattern
        elif name in ['WTI_ÖL', 'KUPFER']:
            if current_gmsi > gmsi_sma and current_ratio < ratio_sma and rsi < 70: return 'LONG', pattern
            if current_gmsi < gmsi_sma and current_ratio > ratio_sma and rsi > 30: return 'SHORT', pattern
        return 'HOLD', pattern
...
st.title("💰 Multi-Commodity Ultra Terminal Lvl 4.2")
...
with col_main:
    if st.button("🚀 LEVEL 4 VOLL-ANALYSE STARTEN"):
        ...
        vix_val, yields_val, fx_val = df['^VIX'].iloc[-1], df['^TNX'], df['CHFUSD=X'].iloc[-1]
        results = []
        for name, ticker in engine.assets.items():
            ...
            results.append({'name': name, 'price': price, 'atr': atr, 'sig': sig, 'rsi': rsi, 'ratio': price/gmsi_val, 'ratio_sma': (price_s/gmsi_series).rolling(20).mean().iloc[-1], 'pattern': pat, 'smart': smart, 'fx': fx_val})
    ...
    if st.session_state['analysis_results']:
        ...
        for res in res_list:
            name, price, sig, rsi, atr, pat, smart, fx = res['name'], res['price'], res['sig'], res['rsi'], res['atr'], res['pattern'], bool(res['smart']), res['fx']
            ...
            with st.expander(f"{'🔵 ' if is_active else '✅ ' if 'HOLD' not in sig else '📊 '}{name} | {pat}", expanded=(sig != 'HOLD' or is_active)):
                ...
                units = (einsatz_chf * lev * fx) / price
                sl, tp, tsl = (price - atr*2, price + atr*4, price - atr*1.5) if clean_dir == "LONG" else (price + atr*2, price - atr*4, price + atr*1.5)
                t1, t2, t3 = st.columns(3)
                t1.markdown(f'<p class="order-label">Einsatz (CHF)</p><p class="order-value">{einsatz_chf:.2f}</p>', unsafe_allow_html=True)
                t2.markdown(f'<p class="order-label">T212 ANZAHL (Units)</p><p class="order-value" style="color:#3b82f6">{units:.4f}</p>', unsafe_allow_html=True)
