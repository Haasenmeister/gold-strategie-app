    ...
    def fetch_data(self):
        ...
        for asset_name, ticker in self.assets.items():
            ...
            if not live_data.empty:
                data.loc[data.index[-1], ticker] = live_data.iloc[-1]
...
with col_main:
    if st.button("🔍 ALLE ROHSTOFFE ANALYSIEREN"):
        ...
        for res in results:
            ...
            with st.expander(f"{'✅ ' if is_ready else '📊 '}{name} | {mode}", expanded=is_ready):
                ...
                st.warning(f"Trailing SL Empfehlung: ${tsl_v:.2f} (Zieht bei Profit nach)")
